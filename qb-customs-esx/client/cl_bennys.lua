...
local function saveVehicle()
    ...
    TriggerServerEvent('qb-custom:server:updateVehicle', myCar)
end
...
function AttemptPurchase(type, upgradeLevel)
    ...
    TriggerServerEvent("qb-customs-esx:server:attemptPurchase", type, upgradeLevel)
...
function GetLocations()
    ESX.TriggerServerCallback("qb-customs-esx:server:GetLocations", function(locations)
...
function CheckForGhostVehicle()
    ...
    local closestVehicle, closestDistance = ESX.Game.GetClosestVehicle(GetEntityCoords(PlayerPedId()))
...
function SetupInteraction()
    ...
    if Config.UseRadial then
        if not radialMenuItemId then
            radialMenuItemId = exports['qb-radialmenu']:AddOption({
                ...
                icon = 'wrench',
                type = 'client',
                event = 'qb-customs-esx:client:EnterCustoms',
                shouldClose = true
            })
        end
    else
    ...
    ESX.ShowHelpNotification("Press ~INPUT_CONTEXT~ to open Motorworks!")
...
AddEventHandler('onResourceStart', function(resourceName)
    if resourceName == GetCurrentResourceName() and ESX.GetPlayerData() ~= {} then
        GetLocations()
    end
end)
...
RegisterNetEvent('qb-customs-esx:client:UpdateLocation', function(location, type, key, value)
    Config.Locations[location][type][key] = value
end)

RegisterNetEvent("qb-customs-esx:client:purchaseSuccessful", function()
    isPurchaseSuccessful = true
    attemptingPurchase = false
   -- QBCore.Functions.Notify("Purchase Successful")
end)

RegisterNetEvent("qb-customs-esx:client:purchaseFailed", function()
    isPurchaseSuccessful = false
    attemptingPurchase = false
    --QBCore.Functions.Notify("Not enough money", "error")
end)

RegisterNetEvent('qb-customs-esx:client:EnterCustoms', function(override)
